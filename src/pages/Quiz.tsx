                     ...
                     <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        {question.options?.map((option: any) => (
                           <div 
                             ...
                             onClick={() => handleSelect(option.id)}
                             className={cn(
                               "p-6 rounded-2xl border-2 cursor-pointer transition-all duration-300 flex flex-col items-center text-center gap-4 bg-white hover:shadow-lg",
                               answers[question.id] === option.id ? "border-accent bg-accent/5" : "border-gray-100"
                             )}
                           >
                              <div className={cn("w-24 h-24 p-4 rounded-xl", answers[question.id] === option.id ? "text-accent" : "text-gray-400")}>
